...
public class AntZipUtil {
	...
	private static List listFile(String path) {
		...
		if (array.length > 0) {
			for (int i = 0; i < array.length; i++) {
				sTemp = path + File.separator + array[i];
				file = new File(sTemp);
				if (file.isDirectory()) {
					listFile(sTemp + "/");
				} else
	...
	public static void zip(String needtozipfilepath, String zipfilepath, String zipName) {
		try {
			byte[] b = new byte[512];
...
//			}
			...
			FileOutputStream fileOutputStream = new FileOutputStream(zipFile + File.separator + zipName);
